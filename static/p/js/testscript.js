(function(){
	...
	var replacedComms = {
		...
		//if it doesnt exist, create it and set value = 100
		modify_status_value : function(statusId, changeVal, cb){
			if(statuses[statusId] === undefined){
				statuses[statusId] = 100;
			}
			statuses[statusId] += changeVal;
			
			cb({"id": statusId, "newValue": statuses[statusId]});
		},
    ...
    (function(proto){
        proto.finishGame = function(score, currency){
            console.log("GAME FINISHED. Score: " + score.toString() + ", Currency: " + currency.toString());
        };
